function [ data, labels_E, labels_ab, labels_mode, labels_phase ] ...
     ...
     %preallocation
     Direct = zeros(num*num, traindata_size);
     ex_xy = zeros(2*num*num, traindata_size);       
     labels_ab = zeros(traindata_size,2);
     labels_mode = zeros(traindata_size,1);
     labels_phase = zeros(traindata_size,1);
     
     
    for iter = 1:1:traindata_size
        mode_xy = randi([0 4],1);
        a = 20 * rand;
        b = 20 * rand;
        phi = k*a/num * randi([0,100],1)/100;
     ...
     data = reshape(Direct, num*num, traindata_size);
     data = data.';
     labels_E = reshape(ex_xy, 2*num*num, traindata_size);
     labels_E = labels_E.';
